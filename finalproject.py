...
tdf = pd.DataFrame()
tdf['ModelName'] = ['XGBoost', 'RandomForest', 'Bagging', 'LinearRegression', 'Ridge']
tdf.set_index('ModelName', inplace=True)
print(tdf)

modelsavedir = "D:\\Desktop\\Python\\Data\\final report\\models"
...
for fileName in filecontant:
    ...
    df = pd.DataFrame(columns=['ModelName', fileName[8:-4] + 'Score'])  # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    # 不做处理
    models = [XGBRegressor(), RandomForestRegressor()]
    models_str = ['XGBoost', 'RandomForest']
    score_ = []
    ...
    for name, model in zip(models_str, models):
        ...
        tmpdf = pd.DataFrame({'ModelName': name, fileName[8:-4] + 'Score': str(score)}, index=['0'])
        df = df.append(tmpdf, ignore_index=True)
        joblib.dump(model, modelsavedir+'\\'+fileName[8:-4]+'_'+name+'.pkl')
